...
class cvvdp(vq_metric):
    ...
    def do_pooling_and_jods(self, Q_per_ch):
        ...
        # DEBUG: Print Q_sc (spatial-channel aggregated scores)
        if Q_per_ch.shape[0] > 0:  # batch_sz > 0
            Q_sc_sample = Q_sc[0,:,0]  # batch=0, frame=0
            print(f"DEBUG_PY_AGG: Q_sc={Q_sc_sample.cpu().numpy()}", flush=True)
            print(f"DEBUG_PY_AGG: beta_sch={self.beta_sch}, beta_tch={self.beta_tch}", flush=True)
        ...
        # DEBUG: Print Q_tc and final Q
        if Q_tc.numel() > 0:
            print(f"DEBUG_PY_AGG: Q_tc (before frame pooling)={Q_tc.cpu().numpy() if Q_tc.dim() > 0 else Q_tc.item()}", flush=True)

        Q = Q.squeeze()
        print(f"DEBUG_PY_AGG: Q_per_ch (final)={Q.item() if Q.dim() == 0 else Q.cpu().numpy()}", flush=True)
    ...
    def process_block_of_frames(self, R, vid_sz, temp_ch, lpyr, is_image):
        ...
        for bb in range(lpyr.get_band_count()):  # For each spatial frequency band
            ...
            # DEBUG: Print per-band scores
            if bb < 10:  # Only print first 10 bands
                band_scores = Q_per_ch_block[0,:,0,bb]  # batch=0, frame=0
                print(f"DEBUG_PY_BAND[{bb}]: band_scores={band_scores.cpu().numpy()}", flush=True)
